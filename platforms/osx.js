...
function downloadAndExtractNWJS(debug) {
    //mkdir(nwjs_download_cache)
    // cd into cache dir

    var url = "https://dl.nwjs.io/v$VERSION$/nwjs$DEBUG$-v$VERSION$-osx-x64.zip"
        .replace('$DEBUG$', debug ? '-sdk' : '')
        .replace('$VERSION$', shared.NWJS_VERSION);
    var zip = url.split('/').pop();
    // check if zip exists

    // check if extracted dir exists
    // extract zip
    // return dirname of extracted zip
...
module.exports = {
    ...
    processor: {
        ...
        makeBinary: function(options) {
            ...
            var NWJS_APP_DIR = BINARY_OUTPUT_DIR + '/nwjs.app/Contents/Resources/app.nw';
